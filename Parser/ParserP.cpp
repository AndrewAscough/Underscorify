...
if ( symbolWord [ i ]   ==  34  ||  symbolWord [ i ]   ==  39 )   {  result += symbolWord [ i ]  ; 
isString ++  ; 
 }   	  	  	 //Section is not a string but IS an operand so it lumps the two symbols together and moves on
else if (  ( isOperand ( symbolWord [ i ]  )   &&  isOperand ( symbolWord [ i + 1 ]  )  )   &&  isString % 2  ==  0 )   {   	  	  	  	 //std::cerr<<std::endl<<"OPERAND FOUND AT " << i << " IT WAS: " << symbolWord[i] << symbolWord[i+1] << std::endl;
result += " " ; 
result += symbolWord [ i ]  ; 
