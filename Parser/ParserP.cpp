#include "Parser.h"
Parsify :: Parser :: Parser ( int argc ,  char *  argv [  ]  )   {  isString  =  0 ; 
isComment  =  0 ; 
for ( int i = 1 ; i < argc ; i ++  ) 
 {  std :: string s ( argv [ i ]  )  ; 
files . push_back ( s )  ; 
 }   }  void Parsify :: Parser :: Parse (  )   {   	 //Opens each file
...
if ( line [ 0 ]   ==  '#'  &&  isComment % 2  == 0 )   {  outputFile << line << std :: endl ; 
...
else if (  ( isBlockComment ( line )   ||  isComment % 2  ==  1 )   &&  isString % 2  ==  0 )   {  outputFile << extractBlockComment ( line )  << std :: endl ; 
 }   	  	  	 //Splits and spaceifys whatever comes before the //comment
else if ( isLineComment ( line )  )   {  outputFile << extractLineComment ( line )  << std :: endl ; 
 }   	  	  	 //Line does not contain a comment so must be examined
else  {  while ( currentLine  >>  word )   {   	  	  	  	  	 //for some resemblance of "formatting" when a ; is encountered it will put it on a new line
if ( newLineNeeded ( word )  )   {  outputFile << spaceifySymbolWord ( word )  << std :: endl ; 
 }   	  	  	  	  	 //Handles breaking up symbol words and passing through strings without changing the interior.
else if ( isSymbolWord ( word )  )   {  outputFile << spaceifySymbolWord ( word )  << " " ; 
...
std :: string Parsify :: Parser :: spaceifySymbolWord ( std :: string symbolWord )   {  std :: string result ; 
for ( int i = 0 ; i < symbolWord . length (  )  ; i ++  ) 
 {   	  	 //Encountered part of the string is a symbol.
if ( isSymbol ( symbolWord [ i ]  )  )   {   	  	  	 //If a \ is encountered make sure to add that and the next character to the result before moving on.
if ( symbolWord [ i ]   ==  '\\' )   {  result += symbolWord [ i ]  ; 
i ++  ; 
result += symbolWord [ i ]  ; 
 }   	  	  	 //Checks to see if the symbolword is entering/exiting a string. This is to handle a string like: "hellothere&&"; or '%'
else if ( symbolWord [ i ]   ==  34  ||  symbolWord [ i ]   ==  39 )   {  result += symbolWord [ i ]  ; 
isString ++  ; 
...
std :: string Parsify :: Parser :: extractLineComment ( std :: string line )   {  std :: string result ; 
std :: string remainderString ; 
for ( int i = 0 ; i < line . length (  )  ; i ++  ) 
 {  if ( line [ i ]   ==  '/'  &&  line [ i + 1 ]   ==  '/' )   {  while ( i < line . length (  )  )   {  result += line [ i ]  ; 
i ++  ; 
 }   }  else  {  remainderString += line [ i ]  ; 
...
std :: string Parsify :: Parser :: extractBlockComment ( std :: string line )   {  std :: string result ; 
std :: string buffer ; 
for ( int i = 0 ; i < line . length (  )  ; i ++  ) 
 {   	  	 //String is encountered before the comment block
if (  ( line [ i ]   ==  34  ||  line [ i ]   ==  39 )   &&  isComment % 2  == 0 )   {  buffer += line [ i ]  ; 
i ++  ; 
...
else if (  ( line [ i ]   ==  '/'  &&  line [ i + 1 ]   ==  '*' )   &&  isComment % 2  ==  0  &&  isString % 2  ==  0 )   {   	  	  	 //Add current word to result after separating out the symbolwords
result +=  spaceifySymbolWord ( buffer )  ; 
buffer  = "" ; 
...
else if (   ( line [ i ]   ==  '*'  &&  line [ i + 1 ]   ==  '/' )   &&  isComment % 2  ==  1  &&  isString % 2  ==  0 )   {   	  	  	 //add buffer to result
result +=  buffer ; 
buffer = "" ; 
