...
#include <fstream>
#include <vector>
namespace Parsify  {  class Parser  {  public :  Parser ( int argc ,  char  * argv [  ]  )  ; 
...
void Parse (  )  ; 
protected :   	  	  	 //Stores list of filenames to be parsed
std :: vector < std :: string >  files ; 
 	  	  	 //Determins if input is in a comment block or not. if isComment%2==0, it is not a comment
int isComment ; 
private :   	  	  	 //Determines if input is currently interacting with a string. if isString%2==0, it is not a string
int isString ; 
